...
const SLUGS = ["product-package", "receipt", "signage"];

describe("use-cases MDX batch3: 製品パッケージ・レシート・看板", () => {
  ...
  it("全9記事のorderが重複しないこと", () => {
    const allSlugs = getUseCaseSlugList();
    const orders = allSlugs.map((slug) => getUseCase(slug)?.order).filter((o) => o !== undefined);
    const uniqueOrders = new Set(orders);
    expect(uniqueOrders.size).toBe(orders.length);
  });
});
